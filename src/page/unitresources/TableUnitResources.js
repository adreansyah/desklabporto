...
import { requestListUnitResources, requestDeleteUnitResources } from 'store/actions/unitresource';
import ModalSmall from 'component/ModalCustom/modalSmall';
import DataTable from 'component/DataTable';
import UpdateUnitResources from './UpdateUnitResources';

const TableUnitResources = () => {
    ...
    const payload = useSelector(state => {
        return {
            ...
            isLoading: state.unitResources.isLoading,
            totalPages: state.unitResources.totalPages,
            parameter: state.unitResources.parameter
        }
    });
    const handleRemove = () => {
        hasFetch(requestDeleteUnitResources(passId))
        onClose('delete');
    }
    useEffect(() => {
        hasFetch(requestListUnitResources(isTable));
        ...
        <>
            <DataTable
                ...
                createSearchAbles={(e) => {
                    ...
                    e.target.value.length > 3 && hasFetch(requestListUnitResources(query));
                    e.target.value.length === 0 && hasFetch(requestListUnitResources(query));
                }}
            />
            <ModalSmall
                isOpen={isToogle.update}
                onClose={() => onClose('update')}
                title={<Text className="u-tx-center">Upadate Unit Resources</Text>}
                content={<UpdateUnitResources id={passId} onClose={() => onClose('update')} />}
            />
            <ModalSmall
                isOpen={isToogle.delete}
                onClose={() => onClose('delete')}
                title={<Text className="u-tx-center">Delete Unit Resources</Text>}
...
export default TableUnitResources;